...
def group_bwd_W(DY, X, expert_offsets, E):
    ...
    BLOCK_N, BLOCK_K, BLOCK_M = 128, 128, 32
    num_stages, num_warps = 4, 4
    M, N, K = DY.size(0), DY.size(-1), X.size(-1)
    NO_K_MASK = (K % BLOCK_K) == 0
    NO_N_MASK = (N % BLOCK_N) == 0
...
class SMoeBlock(MixtralSparseMoeBlock):
    def __init__(self, config):
        super(SMoeBlock, self).__init__(config)
        self.config = config
        self.experts = GLUMLP(
            config.hidden_size,
    ...
    def copy_weights_from_hf(self, hf_model):
        hf_experts = hf_model.experts
        smoe_expert_l1 = self.experts.experts
        smoe_expert_l2 = self.experts.output_experts
        # import pdb; pdb.set_trace()
        for i in range(self.num_experts):
            smoe_expert_l1.weight.data[i] = torch.concat((hf_experts[i].w3.weight.data, hf_experts[i].w1.weight.data), dim=0)
            smoe_expert_l2.weight.data[i] = hf_experts[i].w2.weight.data

        self.gate.weight.data = hf_model.gate.weight.data